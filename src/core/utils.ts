...
export function transIconifyJson(
  o: IconfontConfig,
  jsonStr: string,
  jsStr: string,
): string {
  if (o.iconifyJson) {
    ...
    const svgArr = symbols.map(
      (symbol: { children: any[], properties: { id: any } }) => {
      ...
      icons: Object.fromEntries(
        json.glyphs.map((g: { font_class: any, name: any }) => {
          const { body }
            = svgArr.find(
              (s: { id: string }) =>
                s.id === `${json.css_prefix_text}${g.font_class}`,
            ) || {}
...
export async function generateJson(
  ...
  if (o.iconJson) {
    ...
    generateFile(
      join(root, iconJsonPath).split('\\').join('/'),
      transIconifyJson(o, JSON_CONTENT, JS_CONTENT),
    )
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    http
      .get(targetURL, (res: any) => {
        ...
        res.on(
          'data',
          (chunk: { toString: () => string }) => (data += chunk.toString()),
        )
